...
function CatalogBeforeLogin() {
  ...
  // DITAMBAHKAN: State untuk data produk dari API
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // State kategori & kata kunci pencarian
  const [selectedCategory, setSelectedCategory] = useState("all");
  const [searchQuery, setSearchQuery] = useState("");

  // DITAMBAHKAN: Effect untuk Fetch Data dari API saat mount
  useEffect(() => {
    let mounted = true;
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          const msg = json?.message || `Gagal memuat produk (status ${res.status})`;
          throw new Error(msg);
        }
  ...
  // Baca query string (kategori & search) saat URL berubah
  useEffect(() => {
  ...
  // Filter produk berdasarkan kategori + teks (menggunakan data products dari API)
  ...
  return (
    <div className="font-poppins bg-[#F8FAF7] min-h-screen flex flex-col pt-14 sm:pt-16">
      ...
      {/* Judul + dropdown (sejajar horizontal, termasuk mobile) */}
      ...
      {/* Grid produk (layout & ukuran card sama dengan HomeContent / CatalogAfterLogin) */}
      <section className="w-full bg-[#F8FAF7] pb-14 md:pb-16">
        <div className="max-w-[1380px] mx-auto px-4 sm:px-6 lg:px-10">
          {/* DITAMBAHKAN: Loading dan Error State */}
          {loading && (
            <div className="col-span-full text-center text-sm text-[#3A5A40] py-10">Memuat produk...</div>
          )}

          {error && (
            <div className="col-span-full text-center text-sm text-red-600 py-10">Error: {error}</div>
          )}
          ...
          {!loading && !error && (
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-8 gap-x-4 md:gap-y-12 md:gap-x-8 items-stretch">
              {filteredProducts.map((product, index) => (
                <Link key={product.product_id ?? index} to={`/product-before/${product.product_id ?? index}`}>
                  {/* Card produk: flex + h-full + clamp seperti HomeContent */}
